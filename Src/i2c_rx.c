...
int main() {
	...
	while (1) {
		type = 0x01;
		I2C_MasterSendData(&I2C_Handle, &type, 1, SLAVE_ADDR, I2C_DISABLE_SR);
		I2C_MasterReceiveData(&I2C_Handle, &once, 1, SLAVE_ADDR, I2C_DISABLE_SR);
		for (i = 0; i < 300000; i++);

		type = 0x02;
		I2C_MasterSendData(&I2C_Handle, &type, 1, SLAVE_ADDR, I2C_DISABLE_SR);
		I2C_MasterReceiveData(&I2C_Handle, (uint8_t *)buffer, 14, SLAVE_ADDR, I2C_DISABLE_SR);
		while (1);
	}
